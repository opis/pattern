...
class DefaultTest extends TestCase
{
    ...
    protected function runBuilder(RegexBuilder $builder, string $pattern, array $placeholders = [], string $expected = null, array $tests = null)
    {
        ...
        if ($tests) {
            foreach ($tests as $test => $valid) {
                $this->assertEquals($valid, (bool) preg_match($regex, $test), $test . ($valid ? '' : ' not ') . ' matches ' . $pattern);
            }
        }
    }
    ...
    public function pathProvider()
    {
        return [
            ...
            ['pre-{a?}/test/{b?}-suf', ['a' => '\d', 'b' => '[a-z]'], '~^pre\-(?P<a>(?:\d))?/test/(?P<b>(?:[a-z]))?\-suf/?$~u', [
            ...
            ['{a}{b}/{c?}{d?}', ['a' => '\d', 'b' => '[a-z]', 'c' => '\d', 'd' => '[a-z]'], '~^(?P<a>(?:\d))(?P<b>(?:[a-z]))/(?P<c>(?:\d))?(?P<d>(?:[a-z]))?/?$~u', [
                '2a/3z' => true,
                '5b/0t/' => true,
            ...
            ['{a}{b}/{cd?}', ['a' => '\d', 'b' => '[a-z]', 'cd' => '\d?[a-z]?'], '~^(?P<a>(?:\d))(?P<b>(?:[a-z]))(?:/(?P<cd>(?:\d?[a-z]?)))?/?$~u', [
                '2a/3z' => true,
                '5b/0t/' => true,
                '8c' => true,
                '8c/' => true,
    ...
    public function domainProvider()
    {
        return [
            ...
            ['{a?}.ex{n?}.c', ['a' => '[a-z0-9]+', 'n' => '\d'], '~^(?:(?P<a>(?:[a-z0-9]+))\.)?ex(?P<n>(?:\d))?\.c\.?$~u', [
                'a.ex0.c' => true,
                'a.ex0.c.' => true,
                'a0.ex3.c' => true,
                'a0.ex3.c.' => true,
